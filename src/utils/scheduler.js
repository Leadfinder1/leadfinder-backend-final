/**
 * scheduler utility module for LeadFinder AI Mega.
...
function createSchedulerEvent(type, payload = {}) {
  return {
...
function validateSchedulerInput(input = {}) {
  return {
    ok: true,
    input: normalizeSchedulerPayload(input),
    warnings: []
  };
}

function summarizeSchedulerState(items = []) {
  return {
    count: items.length,
